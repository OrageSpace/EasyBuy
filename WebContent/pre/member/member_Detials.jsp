...
<body>
	...
	<table border="0" class="order_tab"
		...
		<c:forEach var="order" items="${requestScope.pageObj.list}">
			<tr>
				<td align="left"><img src="${ctx}/files/${order.product.fileName}" alt="${order.product.name}" style="vertical-align: middle;" width="50px" height="80px;"/><font color="#ff4e00" >${order.product.name}</font></td>
				<td>${order.orderDetail.quantity}</td>
				<td>￥${order.orderDetail.cost}</td>
			</tr>
		</c:forEach>
	</table>
